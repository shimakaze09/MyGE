...
template <typename Value>
void ReadVar(Value& var, const rapidjson::Value& jsonValueField,
...
void ReadVar(Value& var, const rapidjson::Value& jsonValueField,
  ...
  if constexpr (std::is_floating_point_v<Value>)
    ...
    for (size_t i = 0; i < N; i++)
      ReadVar(ArrayTraits_Get(var, i), arr[static_cast<rapidjson::SizeType>(i)],
              ctx);
  } else if constexpr (OrderContainerTraits<Value>::isOrderContainer) {
    const auto& arr = jsonValueField.GetArray();
    ...
    if constexpr (std::is_same_v<MapTraits_KeyType<Value>, std::string>) {
      ...
      for (const auto& [val_key, val_mapped] : m) {
        ...
        MapTraits_Emplace(var, MapTraits_KeyType<Value>{val_key.GetString()},
                          std::move(mapped));
      }
    } else {

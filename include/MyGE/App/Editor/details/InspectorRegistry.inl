...
//	};
//
//	template<typename T, size_t N> struct ValNTraits<point <T, N>> :
// ValNTraitsBase<T> {}; 	template<typename T, size_t N> struct
// ValNTraits<scale <T, N>> : ValNTraitsBase<T> {}; 	template<typename T,
// size_t N> struct ValNTraits<val   <T, N>> : ValNTraitsBase<T> {};
// template<typename T, size_t N> struct ValNTraits<vec   <T, N>> :
// ValNTraitsBase<T> {}; 	template<typename T>           struct
// ValNTraits<euler <T   >> : ValNTraitsBase<T> {}; 	template<typename T>
// struct
// ValNTraits<normal<T   >> : ValNTraitsBase<T> {}; 	template<typename T>
// struct ValNTraits<quat <T
//>> : ValNTraitsBase<T> {}; 	template<typename T>           struct
// ValNTraits<rgb   <T   >> : ValNTraitsBase<T> {}; 	template<typename T>
...
// InspectorRegistry::InspectContext ctx); 	template<typename Field,
// typename Value> 	void InspectVar(Field field, Value& var,
// InspectorRegistry::InspectContext ctx); 	template<typename Field,
// typename Value> 	bool InspectVar1(Field field, Value& var,
...
//			if
//(ImGui::CollapsingHeader(USRefl::TypeInfo<Cmpt>::name)) {
//				USRefl::TypeInfo<Cmpt>::ForEachVarOf(*cmpt,
//[ctx](auto field, auto& var) {
...
//		else {
//			if
//(ctx.inspector.IsRegistered(TypeID_of<Cmpt>.HashCode()))
//				ctx.inspector.Visit(TypeID_of<Cmpt>.HashCode(),
// cmpt, ctx);
//		}
//	}
//
...
//			USRefl::TypeInfo<Asset>::ForEachVarOf(*asset, [ctx](auto
// field, auto& var) { 				InspectField(field, var, ctx);
//			});
//		}
//		else {
//			if
//(ctx.inspector.IsRegistered(typeid(Asset).hash_code()))
//				ctx.inspector.Visit(typeid(Asset).hash_code(),
// asset, ctx);
//		}
//	}
//
...
//(HasDefinition<USRefl::TypeInfo<UserType>>::value) { 			if
//(ImGui::TreeNode(GetFieldName(field).data())) {
...
// InspectorRegistry::InspectContext ctx) { 		if constexpr
// (std::is_same_v<Value, bool>) { 			ImGui::Button(var ?
// "true" : "false", &var); 			ImGui::SameLine();
//			ImGui::Text(GetFieldName(field).data());
//		}
...
// is_instance_of_v<Value, std::shared_ptr> || is_instance_of_v<Value,
// USTL::shared_object>) { 			ImGui::Text("(* const)");
// ImGui::SameLine(); 			if (var) {
// if constexpr (std::is_pointer_v<Value>)
// InspectVar(field, *var, ctx); 				else {
...
// if (!path.empty()) {
// auto name = path.stem().string(); ImGui::Text(name.c_str());
//						}
//						else
//							ImGui::Text("UNKNOW");
//					}
...
//
//					if constexpr (N == 3)
//						ImGui::ColorEdit3(GetFieldName(field).data(),
// data, flags); 					else // N == 4
// ImGui::ColorEdit4(GetFieldName(field).data(), data, flags);
//				}
//				else {
//					using ElemType =
// ArrayTraits_ValueType<Value>; 					if
// constexpr (std::is_same_v<ElemType, uint8_t>)
//						ImGui::DragScalarN(GetFieldName(field).data(),
// ImGuiDataType_U8, data, N, 1.f); 					else if
// constexpr (std::is_same_v<ElemType, uint16_t>)
// ImGui::DragScalarN(GetFieldName(field).data(), ImGuiDataType_U16, data,
// N, 1.f); 					else if constexpr
// (std::is_same_v<ElemType, uint32_t>)
//						ImGui::DragScalarN(GetFieldName(field).data(),
// ImGuiDataType_U32, data, N, 1.f); 					else if
// constexpr (std::is_same_v<ElemType, uint64_t>)
// ImGui::DragScalarN(GetFieldName(field).data(), ImGuiDataType_U64, data,
// N, 1.f); 					else if constexpr
// (std::is_same_v<ElemType, int8_t>)
//						ImGui::DragScalarN(GetFieldName(field).data(),
// ImGuiDataType_S8, data, N, 1.f); 					else if
// constexpr (std::is_same_v<ElemType, int16_t>)
// ImGui::DragScalarN(GetFieldName(field).data(), ImGuiDataType_S16, data,
// N, 1.f); 					else if constexpr
// (std::is_same_v<ElemType, int32_t>)
//						ImGui::DragScalarN(GetFieldName(field).data(),
// ImGuiDataType_S32, data, N, 1.f); 					else if
// constexpr (std::is_same_v<ElemType, int64_t>)
// ImGui::DragScalarN(GetFieldName(field).data(), ImGuiDataType_S64, data,
// N, 1.f); 					else if constexpr
// (std::is_same_v<ElemType, float>)
//						ImGui::DragScalarN(GetFieldName(field).data(),
// ImGuiDataType_Float, data, N, 0.001f);
// else if constexpr (std::is_same_v<ElemType, double>)
//						ImGui::DragScalarN(GetFieldName(field).data(),
// ImGuiDataType_Double, data, N, 0.001f);
//				}
//			}
//			else {
...
//			if (ImGui::TreeNode(GetFieldName(field).data())) {
//				ImGui::PushID(GetFieldName(field).data());
//				USTL::tuple_for_each(var, [ctx, idx = 0](auto&
// ele) mutable { 					auto str =
// std::to_string(idx++);
...
//		}
//		else if constexpr
//(OrderContainerTraits<Value>::isOrderContainer) { 			if
//(ImGui::TreeNode(GetFieldName(field).data())) {
//				ImGui::PushID(GetFieldName(field).data());
//				auto iter_begin =
// OrderContainerTraits_Begin(var); 				auto iter_end =
// OrderContainerTraits_End(var); 				size_t idx = 0;
// for (auto iter = iter_begin; iter != iter_end; ++iter) {
// auto name = std::to_string(idx++);
// InspectVar(GenerateNameField(name), *iter,
// ctx);
//				}
//				ImGui::PopID();
//				ImGui::TreePop();
...
// InspectorRegistry::InspectContext ctx) { 		if (idx != Idx)
// return false;
//
...
//	void InspectVariant(Field field, Variant& var,
// std::index_sequence<Ns...>, InspectorRegistry::InspectContext ctx) {
//		(InspectVariantAt<Ns>(field, var, var.index(), ctx), ...);
//	}
//
//	template<size_t Idx, typename Field, typename Variant>
//	bool InspectVariantAt1(bool& changed, Field field, Variant& var, size_t
// idx, InspectorRegistry::InspectContext ctx) { 		if (idx != Idx)
// return false;
//
//		using Value = std::variant_alternative_t<Idx, Variant>;
...
//	// TODO : stop
//	template<typename Field, typename Variant, size_t... Ns>
//	bool InspectVariant1(Field field, Variant& var,
// std::index_sequence<Ns...>, InspectorRegistry::InspectContext ctx) {
// bool changed = false; 		(InspectVariantAt1<Ns>(changed, field,
// var, var.index(), ctx), ...); 		return changed;
//	}
//
//	template<typename Field, typename Value>
...
//					ImGui::SliderScalar(GetFieldName(field).data(),
// ImGuiDataType_U16, &var, &minvalue, &maxvalue);
// else if constexpr (std::is_same_v<Value, uint32_t>)
//					ImGui::SliderScalar(GetFieldName(field).data(),
// ImGuiDataType_U32, &var, &minvalue, &maxvalue);
// else if constexpr (std::is_same_v<Value, uint64_t>)
//					ImGui::SliderScalar(GetFieldName(field).data(),
// ImGuiDataType_U64, &var, &minvalue, &maxvalue);
// else if constexpr (std::is_same_v<Value, int8_t>)
//					ImGui::SliderScalar(GetFieldName(field).data(),
// ImGuiDataType_S8, &var, &minvalue, &maxvalue);
// else if constexpr (std::is_same_v<Value, int16_t>)
//					ImGui::SliderScalar(GetFieldName(field).data(),
// ImGuiDataType_S16, &var, &minvalue, &maxvalue);
// else if constexpr (std::is_same_v<Value, int32_t>)
//					ImGui::SliderScalar(GetFieldName(field).data(),
// ImGuiDataType_S32, &var, &minvalue, &maxvalue);
// else if constexpr (std::is_same_v<Value, int64_t>)
//					ImGui::SliderScalar(GetFieldName(field).data(),
// ImGuiDataType_S64, &var, &minvalue, &maxvalue);
// else if constexpr (std::is_same_v<Value, float>)
// ImGui::SliderScalar(GetFieldName(field).data(), ImGuiDataType_Float, &var,
// &minvalue, &maxvalue); 				else if constexpr
//(std::is_same_v<Value, double>)
//					ImGui::SliderScalar(GetFieldName(field).data(),
// ImGuiDataType_Double, &var, &minvalue, &maxvalue); else
// static_assert(false);
//			}
//			else {
//				Value minvalue;
...
//
//				if constexpr (std::is_same_v<Value, uint8_t>)
//					ImGui::DragScalar(GetFieldName(field).data(),
// ImGuiDataType_U8, &var, step, &minvalue); 				else if
// constexpr (std::is_same_v<Value, uint16_t>)
//					ImGui::DragScalar(GetFieldName(field).data(),
// ImGuiDataType_U16, &var, step, &minvalue); 				else if
// constexpr (std::is_same_v<Value, uint32_t>)
//					ImGui::DragScalar(GetFieldName(field).data(),
// ImGuiDataType_U32, &var, step, &minvalue); 				else if
// constexpr (std::is_same_v<Value, uint64_t>)
//					ImGui::DragScalar(GetFieldName(field).data(),
// ImGuiDataType_U64, &var, step, &minvalue); 				else if
// constexpr (std::is_same_v<Value, int8_t>)
// ImGui::DragScalar(GetFieldName(field).data(), ImGuiDataType_S8, &var, step,
// &minvalue); 				else if constexpr (std::is_same_v<Value,
// int16_t>)
// ImGui::DragScalar(GetFieldName(field).data(), ImGuiDataType_S16, &var, step,
// &minvalue); 				else if constexpr (std::is_same_v<Value,
// int32_t>)
// ImGui::DragScalar(GetFieldName(field).data(), ImGuiDataType_S32, &var, step,
// &minvalue); 				else if constexpr (std::is_same_v<Value,
// int64_t>)
// ImGui::DragScalar(GetFieldName(field).data(), ImGuiDataType_S64, &var, step,
// &minvalue); 				else if constexpr (std::is_same_v<Value,
// float>)
// ImGui::DragScalar(GetFieldName(field).data(), ImGuiDataType_Float, &var,
// step,
// &minvalue); 				else if constexpr (std::is_same_v<Value,
// double>)
...
//			if (var.Valid()) {
//				if (auto name =
// ctx.world->entityMngr.Get<Name>(var))
// ImGui::Button(name->value.c_str()); 				else {
...
// sizeof(MyECS::Entity)); 					const auto&
// payload_e = *(MyECS::Entity*)payload->Data; var = payload_e;
//				}
...
//			if constexpr
//(HasDefinition<USRefl::TypeInfo<Value>>::value) {
// std::string_view cur;
// USRefl::TypeInfo<Value>::fields.FindIf([&](auto field) {
// if (field.value == var) { 						cur =
// GetFieldName(field); 						return
// true;
//					}
//					return false;
//				});
...
//			else {
//				InspectVar(field,
// static_cast<std::underlying_type_t<Value>&>(var), ctx);
//			}
//		}
//		else if constexpr (is_instance_of_v<Value, std::shared_ptr> ||
// is_instance_of_v<Value, USTL::shared_object>) { 			using
// Element = typename Value::element_type; 			if constexpr
// (std::is_base_of_v<Object, Element>) {
// ImGui::Text("(*)"); 				ImGui::SameLine();
//				// button
//				if (var) {
//					const auto& path =
// AssetMngr::Instance().GetAssetPath(*var);
// if (!path.empty()) { 						auto
...
//						IM_ASSERT(payload->DataSize ==
// sizeof(xg::Guid)); 						const auto&
// payload_guid = *(const xg::Guid*)payload->Data;
// const auto& path = AssetMngr::Instance().GUIDToAssetPath(payload_guid);
// assert(!path.empty()); 						if (auto
// asset = AssetMngr::Instance().LoadAsset<Element>(path))
// var = asset;
//					}
//					ImGui::EndDragDropTarget();
//				}
...
// else if constexpr (MapTraits<Value>::isMap) { 			if
//(ImGui::TreeNode(GetFieldName(field).data())) {
//				ImGui::PushID(GetFieldName(field).data());
//				auto iter_begin = MapTraits_Begin(var);
//				auto iter_end = MapTraits_End(var);
...
//
//				if constexpr
//(OrderContainerTraits<Value>::isResizable) {
// size_t origSize = OrderContainerTraits_Size(var);
// size_t size = origSize; 					static constexpr
// ImU64 u64_one = 1;
...
//			Value orig = var;
//			if constexpr (std::is_same_v<Value, bool>)
//				ImGui::Checkbox(GetFieldName(field).data(),
//&var); 			else if constexpr (std::is_same_v<Value,
// uint8_t>)
// ImGui::DragScalar(GetFieldName(field).data(), ImGuiDataType_U8,
// &var, 1.f); 			else if constexpr (std::is_same_v<Value,
// uint16_t>)
// ImGui::DragScalar(GetFieldName(field).data(), ImGuiDataType_U16, &var, 1.f);
// else if constexpr (std::is_same_v<Value, uint32_t>)
//				ImGui::DragScalar(GetFieldName(field).data(),
// ImGuiDataType_U32, &var, 1.f); 			else if constexpr
// (std::is_same_v<Value, uint64_t>)
// ImGui::DragScalar(GetFieldName(field).data(), ImGuiDataType_U64, &var, 1.f);
// else if constexpr (std::is_same_v<Value, int8_t>)
//				ImGui::DragScalar(GetFieldName(field).data(),
// ImGuiDataType_S8, &var, 1.f); 			else if constexpr
// (std::is_same_v<Value, int16_t>)
// ImGui::DragScalar(GetFieldName(field).data(), ImGuiDataType_S16, &var, 1.f);
// else if constexpr (std::is_same_v<Value, int32_t>)
//				ImGui::DragScalar(GetFieldName(field).data(),
// ImGuiDataType_S32, &var, 1.f); 			else if constexpr
// (std::is_same_v<Value, int64_t>)
// ImGui::DragScalar(GetFieldName(field).data(), ImGuiDataType_S64, &var, 1.f);
// else if constexpr (std::is_same_v<Value, float>)
//				ImGui::DragFloat(GetFieldName(field).data(),
//&var, 0.001f); 			else if constexpr (std::is_same_v<Value,
// double>)
// ImGui::DragScalar(GetFieldName(field).data(), ImGuiDataType_Double,
// &var, 0.001f); 			else if constexpr (std::is_same_v<Value,
// std::string>)
// ImGui::InputText(GetFieldName(field).data(), &var); 			else if
// constexpr (std::is_same_v<Value, MyECS::Entity>) {
// ImGui::Text("(*)", var.index); ImGui::SameLine();
// if (var.Valid()) { 					if (auto name =
// ctx.world->entityMngr.Get<Name>(var))
// ImGui::Button(name->value.c_str()); 					else {
// std::string str = std::string("Entity (") + std::to_string(var.index) + ")";
//						ImGui::Button(str.c_str());
...
// ImGui::AcceptDragDropPayload(PlayloadType::ENTITY)) {
//						IM_ASSERT(payload->DataSize ==
// sizeof(MyECS::Entity)); 						const
// auto& payload_e = *(MyECS::Entity*)payload->Data;
// var = payload_e;
//					}
//					ImGui::EndDragDropTarget();
//				}
...
// USRefl::TypeInfo<Value>::fields.FindIf([&](auto field) {
// if (field.value == var) {
// cur = GetFieldName(field); return true;
//						}
//						return false;
//					});
//
//					if
//(ImGui::BeginCombo(GetFieldName(field).data(), cur.data())) {
//						USRefl::TypeInfo<Value>::fields.ForEach([&](auto
// field) { 							bool isSelected
// = field.value == var; if (ImGui::Selectable(GetFieldName(field).data(),
// isSelected)) var = field.value;
//
//							if (isSelected)
...
// static_cast<std::underlying_type_t<Value>&>(var), ctx);
//			}
//			else if constexpr (is_instance_of_v<Value,
// std::shared_ptr> || is_instance_of_v<Value, USTL::shared_object>) {
// using Element = typename Value::element_type;
//				static_assert(std::is_base_of_v<Object,
// Element>); 				ImGui::Text("(*)");
// ImGui::SameLine();
//				// button
//				if (var) {
...
//			}
//			else if constexpr (ArrayTraits<Value>::isArray &&
// ValNTraits<Value>::isValN) { 				auto data =
// ArrayTraits_Data(var); 				constexpr auto N =
// ArrayTraits<Value>::size;
//
//				if constexpr (ColorTraits<Value>::isColor) {
//					constexpr ImGuiColorEditFlags flags =
//						ImGuiColorEditFlags_Float
...
//{ ImGui::PushID(GetFieldName(field).data());
// for (size_t i = 0; i < ArrayTraits<Value>::size; i++) {
// auto str = std::to_string(i); if (InspectVar1(GenerateNameField(str),
...
// auto str = std::to_string(idx++); if (InspectVar1(GenerateNameField(str),
// ele, ctx)) 							changed = true;
//					});
//					ImGui::PopID();
//					ImGui::TreePop();
//				}
...
//{ ImGui::PushID(GetFieldName(field).data());
// auto iter_begin = MapTraits_Begin(var);
// auto iter_end = MapTraits_End(var); 					for
// (auto iter = iter_begin; iter != iter_end; ++iter) {
// auto& [key, mapped] = *iter; if constexpr
// (std::is_same_v<std::decay_t<decltype(key)>, std::string>) {
...
//(OrderContainerTraits<Value>::isResizable) {
// size_t origSize = OrderContainerTraits_Size(var);
// size_t size = origSize; 						static
// constexpr ImU64 u64_one = 1;
// ImGui::InputScalar("size", ImGuiDataType_U64, &size, &u64_one);
// if (size !=
// origSize) {
// OrderContainerTraits_Resize(var, size);
...
//
//					auto iter_begin =
// OrderContainerTraits_Begin(var); 					auto
// iter_end = OrderContainerTraits_End(var);
// size_t idx = 0; 					for (auto iter =
// iter_begin; iter != iter_end; ++iter) {
// auto name = std::to_string(idx++); if (InspectVar1(GenerateNameField(name),
//*iter, ctx)) 							changed = true;
//					}
//					ImGui::PopID();
//					ImGui::TreePop();
//				}
...
//		}
//		else if constexpr (is_instance_of_v<Value, std::variant>)
//			return InspectVariant1(field, var,
// std::make_index_sequence<std::variant_size_v<Value>>{}, ctx);
// else { 			Value orig;
// InspectUserType(field, &var, ctx); 			return orig != var;
//		}
//	}
// }

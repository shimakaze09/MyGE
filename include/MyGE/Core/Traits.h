...
template <typename T, size_t N>
struct ArrayTraits<T[N]> : ArrayTraitsBase<N> {};
...
template <typename OrderContainer>
using OrderContainerTraits_ValueType =
    std::decay_t<decltype(*OrderContainerTraits_Begin(
        std::declval<OrderContainer>()))>;
template <typename OrderContainer>
void OrderContainerTraits_Add(
    OrderContainer& container,
    OrderContainerTraits_ValueType<OrderContainer>&& value) {
...
template <typename T, typename Pr, typename Alloc>
void OrderContainerTraits_Add(std::multiset<T, Pr, Alloc>& container,
                              T&& value) {
  container.insert(std::move(value));
}
...
template <typename T, typename Hasher, typename KeyEq, typename Alloc>
void OrderContainerTraits_Add(
    std::unordered_set<T, Hasher, KeyEq, Alloc>& container, T&& value) {
  container.insert(std::move(value));
}

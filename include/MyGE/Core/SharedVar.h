...
class UniqueVar {
 ...
 public:
  ...
  using pointer_to_const =
      std::add_pointer_t<const std::remove_pointer_t<pointer>>;
...
class SharedVar {
 ...
 public:
  ...
  SharedVar(MyDRefl::SharedObject obj) noexcept {
    if (obj.GetType().Is<T>())
  ...
  SharedVar& operator=(MyDRefl::SharedObject obj) noexcept {
    if (obj.GetType().Is<T>())
      ptr = std::reinterpret_pointer_cast<T>(obj.GetBuffer());
    return *this;
  }
...
SharedVar<Ty1> dynamic_var_cast(SharedVar<Ty2>&& other) noexcept {
  return {
      std::dynamic_pointer_cast<Ty1>(std::move(other.cast_to_shared_ptr()))};
}
...
SharedVar<Ty1> reinterpret_var_cast(SharedVar<Ty2>&& other) noexcept {
  return {std::reinterpret_pointer_cast<Ty1>(
      std::move(other.cast_to_shared_ptr()))};
}
...
template <typename Ty1, typename Ty2>
const SharedVar<Ty1> reinterpret_var_cast(
    const SharedVar<Ty2>& other) noexcept {

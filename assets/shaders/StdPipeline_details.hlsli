...
float2 _LTC_CorrectUV(float2 uv) { return uv * _LTC_LUT_SCALE + _LTC_LUT_BIAS; }
...
#define STD_PIPELINE_LTC_SAMPLE_DEFINE                               \
  ...
  float3x3 _LTC_GetInvM_GGX(float2 uv) {                             \
    ...
    return float3x3(t1.x, 0, t1.z, 0, 1, 0, t1.y, 0, t1.w);          \
  }                                                                  \
                                                                     \
  /* GGX norm, Fresnel */                                            \
  float2 _LTC_GetNF_GGX(float2 uv) {                                 \
...
float3 _LTC_IntegrateEdgefloat(float3 v1, float3 v2) {
  ...
  float theta_sintheta =
      (x > 0.0) ? v : 0.5 * rsqrt(max(1.0 - x * x, 1e-7)) - v;
...
float _LTC_Rect_Evaluate(float3 N, float3 V, float3 P, float3x3 invM, float3 p0,
  ...
  if (n > 0) {
    ...
    if (n >= 4) sum += _LTC_IntegrateEdge(L[4], L[3]);
    if (n == 5) sum += _LTC_IntegrateEdge(L[0], L[4]);
    sum = max(0.0, sum);
  }
#endif
  return sum;
...
float3 LTC_Rect_Diffuse(float3 N, float3 V, float3 P, float roughness,
                        float3 p0, float3 p1, float3 p2, float3 p3) {
  float3x3 invM = float3x3(1, 0, 0, 0, 1, 0, 0, 0, 1);
...
float3 _LTC_SolveCubic(float4 Coefficient) {
  ...
  float3 Delta =
      float3(-Coefficient.z * Coefficient.z + Coefficient.y,
...
float3 _LTC_Disk_Evaluate(float3 N, float3 V, float3 P, float3x3 invM,
  ...
  if (dot(cross(V1, V2), C) > 0.0) return float3(0.0, 0.0, 0.0);

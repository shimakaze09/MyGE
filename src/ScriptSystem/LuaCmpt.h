...
class LuaCmpt {
 public:
  LuaCmpt(MyECS::CmptPtr ptr) : ptr{ptr} {}
  LuaCmpt(MyECS::CmptType type, void* ptr) : ptr{MyECS::CmptPtr{type, ptr}} {}

  MyECS::CmptPtr GetCmptPtr() const noexcept { return ptr; }
  ...
  std::string_view GetString(size_t offset, size_t size) {
    return {Offset<char>(offset), size};
  }
  void SetString(size_t offset, std::string_view value) {
    memcpy(Offset(offset), value.data(), value.size());
  }

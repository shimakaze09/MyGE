...
void Mesh::SetSubMeshCount(size_t num) {
  ...
  if (submeshes.size() < num) {
    for (size_t i = submeshes.size(); i < num; i++) {
      submeshes.emplace_back(static_cast<size_t>(-1), static_cast<size_t>(-1));
    }
  } else {
...
void Mesh::SetSubMesh(size_t index, SubMeshDescriptor desc) noexcept {
  if (!IsEditable() || index >= submeshes.size()) return;
  SetDirty();
  vertexBuffer_dirty = true;
...
bool Mesh::IsVertexValid() const noexcept {
  ...
  if (num != 0 && (uv.size() == 0 || uv.size() == num) &&
...
void Mesh::UpdateVertexBuffer() {
  if (!vertexBuffer_dirty) return;

  if (!IsVertexValid()) return;

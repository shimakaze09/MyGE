...
void RenderQueue::Add(RenderObject object) {
  if (object.material->shader->passes[object.passIdx].queue <
      (size_t)ShaderPass::Queue::Transparent)
...
void RenderQueue::Sort(pointf3 cameraPos) {
  auto opaqueLess = [=](const RenderObject& lhs, const RenderObject& rhs) {
    ...
    auto lpos = lhs.mesh->GetSubMeshes().at(lhs.submeshIdx).bounds.center() +
                lhs.translation;
    auto rpos = rhs.mesh->GetSubMeshes().at(rhs.submeshIdx).bounds.center() +
                rhs.translation;

    return pointf3::distance(lpos, cameraPos) >
           pointf3::distance(rpos, cameraPos);
  };

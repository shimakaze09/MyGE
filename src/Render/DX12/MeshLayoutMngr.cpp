...
std::vector<D3D12_INPUT_ELEMENT_DESC> MeshLayoutMngr::GenerateDesc(bool uv,
  ...
  rst.push_back({"POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 0,
                 D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0});
  offset += 12;

  if (uv) {
    rst.push_back({"TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 0, offset,
                   D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0});
    offset += 8;
  } else {
    rst.push_back({"TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 0, 0,
                   D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0});
  }

  if (normal) {
    rst.push_back({"NORMAL", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, offset,
                   D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0});
    offset += 12;
  } else {
    rst.push_back({"NORMAL", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 0,
                   D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0});
  }
  ...
  if (color) {
    rst.push_back({"COLOR", 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, offset,
                   D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0});
    offset += 16;
  } else {
    rst.push_back({"COLOR", 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, 0,
                   D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0});
  }
...
const std::vector<D3D12_INPUT_ELEMENT_DESC>& MeshLayoutMngr::GetMeshLayoutValue(
    size_t ID) {
...
size_t MeshLayoutMngr::GetMeshLayoutID(const Mesh* mesh) noexcept {
  return GetMeshLayoutID(!mesh->GetUV().empty(), !mesh->GetNormals().empty(),

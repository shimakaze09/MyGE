  ...
  for (auto material : materials) {
    ...
    for (size_t i = 0; i < shader.passes.size(); i++) {
      UpdateShaderCBsForRefl(
          flags, *material,
          GPURsrcMngrDX12::Instance().GetShaderRefl_vs(shader, i));
      UpdateShaderCBsForRefl(
          flags, *material,
          GPURsrcMngrDX12::Instance().GetShaderRefl_ps(shader, i));
    }
  }
...
void PipelineBase::SetGraphicsRoot_CBV_SRV(
  ...
  size_t cbPos = buffer->GetResource()->GetGPUVirtualAddress() +
                 shaderCBDescconst.indexMap.at(material.GetInstanceID()) *
    ...
    for (UINT i = 0; i < shaderDesc.BoundResources; i++) {
      ...
      switch (rsrcDesc.Type) {
        ...
        case D3D_SIT_TEXTURE: {
          ...
          if (rootParamIndex == static_cast<size_t>(-1)) break;

          D3D12_GPU_DESCRIPTOR_HANDLE handle;
          handle.ptr = 0;

          if (auto target = material.properties.find(rsrcDesc.Name);
              target != material.properties.end()) {
            auto dim = rsrcDesc.Dimension;
            switch (dim) {
              case D3D_SRV_DIMENSION_TEXTURE2D: {
                assert(std::holds_alternative<std::shared_ptr<Texture2D>>(
                    target->second));
                auto tex2d =
                    std::get<std::shared_ptr<Texture2D>>(target->second);
                handle = GPURsrcMngrDX12::Instance().GetTexture2DSrvGpuHandle(
                    *tex2d);
                break;
              }
              case D3D_SRV_DIMENSION_TEXTURECUBE: {
                assert(std::holds_alternative<std::shared_ptr<TextureCube>>(
                    target->second));
                auto texcube =
                    std::get<std::shared_ptr<TextureCube>>(target->second);
                handle = GPURsrcMngrDX12::Instance().GetTextureCubeSrvGpuHandle(
                    *texcube);
                break;
              }
              default:
          ...
          } else if (auto target = commonSRVs.find(rsrcDesc.Name);
                     target != commonSRVs.end())
  ...
  for (size_t i = 0; i < material.shader->passes.size(); i++) {
    SetGraphicsRoot_Refl(
        GPURsrcMngrDX12::Instance().GetShaderRefl_vs(*material.shader, i));
    SetGraphicsRoot_Refl(
        GPURsrcMngrDX12::Instance().GetShaderRefl_ps(*material.shader, i));
  }
}
...
void PipelineBase::SetPSODescForRenderState(
  ...
  for (size_t i = 0; i < 8; i++) {
    if (renderState.blendStates[i].enable) {
      ...
      desc.BlendState.RenderTarget[i].SrcBlend =
          static_cast<D3D12_BLEND>(renderState.blendStates[i].src);
      desc.BlendState.RenderTarget[i].DestBlend =
          static_cast<D3D12_BLEND>(renderState.blendStates[i].dest);
      desc.BlendState.RenderTarget[i].BlendOp =
          static_cast<D3D12_BLEND_OP>(renderState.blendStates[i].op);
      desc.BlendState.RenderTarget[i].SrcBlendAlpha =
          static_cast<D3D12_BLEND>(renderState.blendStates[i].srcAlpha);
      desc.BlendState.RenderTarget[i].DestBlendAlpha =
          static_cast<D3D12_BLEND>(renderState.blendStates[i].destAlpha);
      desc.BlendState.RenderTarget[i].BlendOpAlpha =
          static_cast<D3D12_BLEND_OP>(renderState.blendStates[i].opAlpha);
    }
  }
  ...
  if (renderState.stencilState.enable) {
    ...
    desc.DepthStencilState.StencilWriteMask =
        renderState.stencilState.writeMask;
    D3D12_DEPTH_STENCILOP_DESC stencilDesc;
    stencilDesc.StencilDepthFailOp =
        static_cast<D3D12_STENCIL_OP>(renderState.stencilState.depthFailOp);
    stencilDesc.StencilFailOp =
        static_cast<D3D12_STENCIL_OP>(renderState.stencilState.failOp);
    stencilDesc.StencilPassOp =
        static_cast<D3D12_STENCIL_OP>(renderState.stencilState.passOp);
    stencilDesc.StencilFunc =
        static_cast<D3D12_COMPARISON_FUNC>(renderState.stencilState.func);

...
void PipelineBase::SetGraphicsRootSRV(ID3D12GraphicsCommandList* cmdList,
  ...
  auto SetSRV = [&](ID3D12ShaderReflection* refl) {
    ...
    auto GetSRVRootParamIndex = [&](size_t registerIndex) {
      for (size_t i = 0; i < material->shader->rootParameters.size(); i++) {
        ...
        if (!std::holds_alternative<RootDescriptorTable>(param)) continue;

        const auto& table = std::get<RootDescriptorTable>(param);
        if (table.size() != 1) continue;

        const auto& range = table.front();
        if (range.NumDescriptors != 1) continue;

        if (range.BaseShaderRegister == registerIndex) return i;
      }
      assert(false);
    ...
    for (UINT i = 0; i < shaderDesc.BoundResources; i++) {
      ...
      if (rsrcDesc.Type != D3D_SIT_TEXTURE) continue;

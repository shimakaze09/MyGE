          ...
          if (auto target = material.properties.find(rsrcDesc.Name);
            ...
            switch (dim) {
              case D3D_SRV_DIMENSION_TEXTURE2D: {
                assert(std::holds_alternative<SharedVar<Texture2D>>(
                    target->second.value));
                auto tex2d =
                    std::get<SharedVar<Texture2D>>(target->second.value);
                handle = GPURsrcMngrDX12::Instance().GetTexture2DSrvGpuHandle(
                    *tex2d);
                break;
              }
              case D3D_SRV_DIMENSION_TEXTURECUBE: {
                assert(std::holds_alternative<SharedVar<TextureCube>>(
                    target->second.value));
                auto texcube =
                    std::get<SharedVar<TextureCube>>(target->second.value);

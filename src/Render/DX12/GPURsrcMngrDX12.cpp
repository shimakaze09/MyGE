...
struct GPURsrcMngrDX12::Impl {
  ...
  unordered_map<std::size_t, MyDX12::MeshGPUBuffer> meshMap;
...
GPURsrcMngrDX12::GPURsrcMngrDX12() : pImpl(new Impl) {}
...
GPURsrcMngrDX12& GPURsrcMngrDX12::RegisterTexture2D(Texture2D& tex2D) {
  ...
  data.RowPitch =
      tex2D.image.GetWidth() * tex2D.image.GetChannel() * sizeof(float);
  data.SlicePitch = tex2D.image.GetHeight() *
                    data.RowPitch;  // this field is useless for texture 2d

  pImpl->hasUpload = true;
  DirectX::CreateTextureFromMemory(
      pImpl->device, *pImpl->upload, tex2D.image.GetWidth(),
      tex2D.image.GetHeight(), channelMap[tex2D.image.GetChannel() - 1], data,
      &tex.resource);

  const auto tex2DSRVDesc =
      MyDX12::Desc::SRV::Tex2D(tex.resource->GetDesc().Format);
  pImpl->device->CreateShaderResourceView(
      tex.resource.Get(), &tex2DSRVDesc,
...
GPURsrcMngrDX12& GPURsrcMngrDX12::UnregisterTexture2D(std::size_t ID) {
  if (auto target = pImpl->texture2DMap.find(ID);
      target != pImpl->texture2DMap.end()) {
...
GPURsrcMngrDX12& GPURsrcMngrDX12::RegisterTextureCube(TextureCube& texcube) {
  ...
  for (size_t i = 0; i < datas.size(); i++) {
    ...
    datas[i].RowPitch = texcube.GetSixSideImages()[i].GetWidth() *
                        texcube.GetSixSideImages()[i].GetChannel() *
                        sizeof(float);
    datas[i].SlicePitch =
        texcube.GetSixSideImages()[i].GetHeight() *
  ...
  pImpl->device->CreateShaderResourceView(
      tex.resource.Get(), &texCubeSRVDesc,
      tex.allocationSRV.GetCpuHandle(static_cast<uint32_t>(0)));

  pImpl->textureCubeMap.emplace_hint(
      target, std::make_pair(texcube.GetInstanceID(), std::move(tex)));

  return *this;
}

GPURsrcMngrDX12& GPURsrcMngrDX12::UnregisterTextureCube(std::size_t ID) {
  if (auto target = pImpl->textureCubeMap.find(ID);
      target != pImpl->textureCubeMap.end()) {
    auto& tex = target->second;
    MyDX12::DescriptorHeapMngr::Instance().GetCSUGpuDH()->Free(
        move(tex.allocationSRV));
    pImpl->deleteBatch.Add(tex.resource.Detach());
...
D3D12_CPU_DESCRIPTOR_HANDLE GPURsrcMngrDX12::GetTexture2DSrvCpuHandle(
...
MyDX12::MeshGPUBuffer& GPURsrcMngrDX12::RegisterMesh(
...
MyDX12::MeshGPUBuffer& GPURsrcMngrDX12::GetMeshGPUBuffer(
...
const ID3DBlob* GPURsrcMngrDX12::GetShaderByteCode_vs(const Shader& shader,
...
const ID3DBlob* GPURsrcMngrDX12::GetShaderByteCode_ps(const Shader& shader,
                                                      size_t passIdx) const {
  return pImpl->shaderMap.at(shader.GetInstanceID())
      .passes[passIdx]
      .psByteCode.Get();
...
ID3D12ShaderReflection* GPURsrcMngrDX12::GetShaderRefl_vs(
    const Shader& shader, size_t passIdx) const {
  return pImpl->shaderMap.at(shader.GetInstanceID())
      .passes[passIdx]
      .vsRefl.Get();
}

ID3D12ShaderReflection* GPURsrcMngrDX12::GetShaderRefl_ps(
    const Shader& shader, size_t passIdx) const {
  return pImpl->shaderMap.at(shader.GetInstanceID())
      .passes[passIdx]
      .psRefl.Get();
...
ID3D12RootSignature* GPURsrcMngrDX12::GetShaderRootSignature(
    const Shader& shader) const {
  return pImpl->shaderMap.at(shader.GetInstanceID()).rootSignature.Get();
}

size_t GPURsrcMngrDX12::RegisterPSO(
...
std::array<CD3DX12_STATIC_SAMPLER_DESC, 6> GPURsrcMngrDX12::GetStaticSamplers()
    const {

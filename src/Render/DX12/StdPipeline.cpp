...
struct StdPipeline::Impl {
  ...
  struct RenderContext {
    ...
    std::unordered_map<const Shader*, PipelineBase::ShaderCBDesc>
        shaderCBDescMap;  // shader ID -> desc
    ...
    size_t objectOffset =
        MyDX12::Util::CalcConstantBufferByteSize(sizeof(CameraConstants)) +
        MyDX12::Util::CalcConstantBufferByteSize(sizeof(LightArray));
    struct EntityData {
      valf<16> l2w;
      valf<16> w2l;
    };
  ...
  size_t GetPSO_ID(std::shared_ptr<const Shader> shader, size_t passIdx,
                   const Mesh& mesh, size_t rtNum, DXGI_FORMAT rtFormat);
...
void StdPipeline::Impl::BuildTextures() {
  ...
  auto blackTex2D = AssetMngr::Instance().LoadAsset<Texture2D>(
      LR"(_internal\textures\black.png)");
  auto blackRsrc =
      GPURsrcMngrDX12::Instance().GetTexture2DResource(*blackTex2D);
  // TODO bugs!
  // auto skyboxBlack =
  // AssetMngr::Instance().LoadAsset<Material>(LR"(_internal\materials\skyBlack.mat)");
  // auto blackTexCube =
  // std::get<SharedVar<TextureCube>>(tmp->properties.find("gSkybox")->second.value);
  // auto tmp = std::make_shared<Material>();
  // tmp->properties.emplace("gSkybox", blackTexCube);
  auto blackTexCube =
      SharedVar<TextureCube>(AssetMngr::Instance().LoadAsset<TextureCube>(
          LR"(_internal\textures\blackCube.png)"));
  auto blackTexCubeRsrc =
      GPURsrcMngrDX12::Instance().GetTextureCubeResource(*blackTexCube);
  defaultSkybox =
      GPURsrcMngrDX12::Instance().GetTextureCubeSrvGpuHandle(*blackTexCube);
...
void StdPipeline::Impl::BuildFrameResources() {
  for (const auto& fr : frameRsrcMngr.GetFrameResources()) {
    ...
    {  // irradiance
      ...
      const auto defaultHeapProp =
          CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT);
      initDesc.device->CreateCommittedResource(
          &defaultHeapProp, D3D12_HEAP_FLAG_NONE, &rsrcDesc,
    ...
    {  // BRDF LUT
      auto brdfLUTTex2D = AssetMngr::Instance().LoadAsset<Texture2D>(
          LR"(_internal\textures\BRDFLUT.png)");
...
void StdPipeline::Impl::BuildShaders() {
  deferLightingShader =
      ShaderMngr::Instance().Get("StdPipeline/Defer Lighting");
...
void StdPipeline::Impl::BuildPSOs() {
  ...
  ID_PSO_defer_light =
      GPURsrcMngrDX12::Instance().RegisterPSO(&deferLightingPsoDesc);
  ...
  ID_PSO_postprocess =
      GPURsrcMngrDX12::Instance().RegisterPSO(&postprocessPsoDesc);
...
size_t StdPipeline::Impl::GetPSO_ID(std::shared_ptr<const Shader> shader,
                                    size_t passIdx, const Mesh& mesh,
                                    size_t rtNum, DXGI_FORMAT rtFormat) {
  ...
  if (target == PSOIDMap.end()) {
    ...
    auto desc = MyDX12::Desc::PSO::MRT(
        ...
        (UINT)rtNum, rtFormat, DXGI_FORMAT_D24_UNORM_S8_UINT);
    desc.RasterizerState.FrontCounterClockwise = TRUE;
...
void StdPipeline::Impl::UpdateRenderContext(
  ...
  {  // camera
    auto cmptCamera =
        cameraData.world.entityMngr.ReadComponent<Camera>(cameraData.entity);
    auto cmptW2L = cameraData.world.entityMngr.ReadComponent<WorldToLocal>(
        cameraData.entity);
    auto cmptTranslation =
        cameraData.world.entityMngr.ReadComponent<Translation>(
            cameraData.entity);
          ...
          [&](ChunkView chunk) {
            ...
            for (size_t i = 0; i < N; i++) {
              ...
              for (size_t j = 0; j < M; j++) {
                ...
                if (material->shader->passes.empty()) continue;
              ...
              if (target != renderContext.entity2data.end()) continue;
              ...
              renderContext.entity2data.emplace_hint(
                  target, std::pair{obj.entity.index, data});
            }
          },
          filter, false);
    }
  ...
  {  // light
    ...
    filter.all = {MyECS::AccessTypeID_of<MyECS::Latest<LocalToWorld>>};
    for (auto world : worlds) {
      world->RunEntityJob(
          [&](const Light* light) {
            switch (light->mode) {
            ...
            switch (light->mode) {
              ...
              case Light::Mode::Rect:
                ...
                renderContext.lights.lights[cur_rectLight].horizontal =
                    (l2w->value * vecf3{1, 0, 0}).safe_normalize();
              ...
              case Light::Mode::Disk:
                ...
                renderContext.lights.lights[cur_diskLight].dir =
                    (l2w->value * vecf3{0, 1, 0}).safe_normalize();
                renderContext.lights.lights[cur_diskLight].horizontal =
                    (l2w->value * vecf3{1, 0, 0}).safe_normalize();
  ...
  for (auto world : worlds) {
    if (auto ptr = world->entityMngr.ReadSingleton<Skybox>();
        ptr && ptr->material &&
        ptr->material->shader.get() == skyboxShader.get()) {
      auto target = ptr->material->properties.find("gSkybox");
      if (target != ptr->material->properties.end() &&
          std::holds_alternative<SharedVar<TextureCube>>(
              target->second.value)) {
        auto texcube = std::get<SharedVar<TextureCube>>(target->second.value);
        renderContext.skybox =
            GPURsrcMngrDX12::Instance().GetTextureCubeSrvGpuHandle(*texcube);
        break;
      }
    }
  }
...
void StdPipeline::Impl::UpdateShaderCBs() {
  auto& shaderCBMngr =
      frameRsrcMngr.GetCurrentFrameResource()->GetResource<ShaderCBMngrDX12>(
          "ShaderCBMngrDX12");
  ...
  {  // camera, lights, objects
    ...
    // light array
    buffer->Set(renderContext.lightOffset, &renderContext.lights,
                sizeof(LightArray));
  ...
  auto AddOpaque = [&](size_t& index) {
    if (opaques.size() == index) return false;
  ...
  std::unordered_map<const Shader*, std::unordered_set<const Material*>>
      transparentMaterialMap;
  for (const auto& transparent : renderContext.renderQueue.GetTransparents())
    transparentMaterialMap[transparent.material->shader.get()].insert(
        transparent.material.get());

  auto Commit = [&]() {
    if (shader == nullptr) return;

    if (auto target = transparentMaterialMap.find(shader.get());
        target != transparentMaterialMap.end()) {
      for (auto material : target->second) materials.insert(material);
      transparentMaterialMap.erase(target);
    }

    renderContext.shaderCBDescMap[shader.get()] = PipelineBase::UpdateShaderCBs(
...
void StdPipeline::Impl::Render(const ResizeData& resizeData,
  ...
  auto gbPass = fg.RegisterPassNode("GBuffer Pass", {},
  ...
  auto deferLightingPass = fg.RegisterPassNode(
      ...
      {deferLightedRT});
  auto skyboxPass =
      fg.RegisterPassNode("Skybox", {deferDS}, {deferLightedSkyRT});
  auto forwardPass = fg.RegisterPassNode(
      "Forward", {irradianceMap, prefilterMap}, {forwardDS, sceneRT});
  auto postprocessPass =
      fg.RegisterPassNode("Post Process", {sceneRT}, {presentedRT});
  ...
  (*fgRsrcMngr)
      ...
      .RegisterPassRsrc(gbPass, gbuffer2, D3D12_RESOURCE_STATE_RENDER_TARGET,
                        rtvNull)
      .RegisterPassRsrc(gbPass, deferDS, D3D12_RESOURCE_STATE_DEPTH_WRITE,
                        dsvDesc)
      ...
      .RegisterPassRsrc(deferLightingPass, gbuffer2,
                        D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE, srvDesc)
      .RegisterPassRsrcState(deferLightingPass, deferDS,
                             D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE |
      ...
      .RegisterPassRsrcState(deferLightingPass, prefilterMap,
                             D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE)
      .RegisterPassRsrc(deferLightingPass, deferLightedRT,
                        D3D12_RESOURCE_STATE_RENDER_TARGET, rtvNull)

      .RegisterPassRsrc(skyboxPass, deferDS, D3D12_RESOURCE_STATE_DEPTH_READ,
                        dsvDesc)
      ...
      .RegisterPassRsrc(forwardPass, sceneRT,
                        D3D12_RESOURCE_STATE_RENDER_TARGET, rtvNull)

      .RegisterPassRsrc(postprocessPass, sceneRT,
                        D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE, srvDesc)
      ...
      iblPass, [&](ID3D12GraphicsCommandList* cmdList,
        ...
        if (iblData->nextIdx < 6) {  // irradiance
          cmdList->SetGraphicsRootSignature(
              GPURsrcMngrDX12::Instance().GetShaderRootSignature(
                  *irradianceShader));
          cmdList->SetPipelineState(
              GPURsrcMngrDX12::Instance().GetPSO(ID_PSO_irradiance));
          ...
          cmdList->SetGraphicsRootSignature(
              GPURsrcMngrDX12::Instance().GetShaderRootSignature(
                  *prefilterShader));
          cmdList->SetPipelineState(
              GPURsrcMngrDX12::Instance().GetPSO(ID_PSO_prefilter));
      ...
      deferLightingPass, [&](ID3D12GraphicsCommandList* cmdList,
        ...
        cmdList->SetGraphicsRootSignature(
            GPURsrcMngrDX12::Instance().GetShaderRootSignature(
                *deferLightingShader));
        cmdList->SetPipelineState(
            GPURsrcMngrDX12::Instance().GetPSO(ID_PSO_defer_light));
  ...
  fgExecutor.RegisterPassFunc(
      skyboxPass, [&](ID3D12GraphicsCommandList* cmdList,
        ...
        cmdList->SetGraphicsRootSignature(
            GPURsrcMngrDX12::Instance().GetShaderRootSignature(*skyboxShader));
        cmdList->SetPipelineState(
            GPURsrcMngrDX12::Instance().GetPSO(ID_PSO_skybox));
  ...
  fgExecutor.RegisterPassFunc(
      forwardPass, [&](ID3D12GraphicsCommandList* cmdList,
                       const MyDX12::FG::PassRsrcs& rsrcs) {
        auto heap = MyDX12::DescriptorHeapMngr::Instance()
                        .GetCSUGpuDH()
                        ->GetDescriptorHeap();
      ...
      postprocessPass, [&](ID3D12GraphicsCommandList* cmdList,
        ...
        cmdList->SetGraphicsRootSignature(
            GPURsrcMngrDX12::Instance().GetShaderRootSignature(
                *postprocessShader));
        cmdList->SetPipelineState(
            GPURsrcMngrDX12::Instance().GetPSO(ID_PSO_postprocess));
...
void StdPipeline::Impl::DrawObjects(ID3D12GraphicsCommandList* cmdList,
                                    std::string_view lightMode, size_t rtNum,
                                    DXGI_FORMAT rtFormat) {
  auto& shaderCBMngr =
      frameRsrcMngr.GetCurrentFrameResource()->GetResource<ShaderCBMngrDX12>(
          "ShaderCBMngrDX12");
  ...
  auto Draw = [&](const RenderObject& obj) {
    ...
    auto& meshGPUBuffer =
        GPURsrcMngrDX12::Instance().GetMeshGPUBuffer(*obj.mesh);
    ...
    cmdList->SetPipelineState(GPURsrcMngrDX12::Instance().GetPSO(
  ...
  for (const auto& obj : renderContext.renderQueue.GetOpaques()) Draw(obj);

  for (const auto& obj : renderContext.renderQueue.GetTransparents()) Draw(obj);
}

...
RsrcMngrDX12& RsrcMngrDX12::RegisterTexture2D(
    DirectX::ResourceUploadBatch& upload, size_t id, const Texture2D* tex2D) {
  return RegisterTexture2DArray(upload, id, &tex2D, 1);
}

RsrcMngrDX12& RsrcMngrDX12::RegisterTexture2DArray(
    DirectX::ResourceUploadBatch& upload, size_t id, const Texture2D** tex2Ds,
    size_t num) {
  Impl::Texture tex;
  tex.resources.resize(num);
  ...
  for (size_t i = 0; i < num; i++) {
    ...
    data.RowPitch =
        tex2Ds[i]->image->width * tex2Ds[i]->image->channel * sizeof(float);
    data.SlicePitch = tex2Ds[i]->image->height * data.RowPitch;

    DirectX::CreateTextureFromMemory(
        pImpl->device, upload, tex2Ds[i]->image->width.get(),
        tex2Ds[i]->image->height.get(),
        channelMap[tex2Ds[i]->image->channel - 1], data, &tex.resources[i]);
...
D3D12_CPU_DESCRIPTOR_HANDLE RsrcMngrDX12::GetTextureSrvCpuHandle(
    size_t id, UINT index) const {
  return pImpl->textureMap.find(id)->second.allocationSRV.GetCpuHandle(index);
}
D3D12_GPU_DESCRIPTOR_HANDLE RsrcMngrDX12::GetTextureSrvGpuHandle(
    size_t id, UINT index) const {
  return pImpl->textureMap.find(id)->second.allocationSRV.GetGpuHandle(index);
}

MyDX12::DescriptorHeapAllocation& RsrcMngrDX12::GetTextureRtvs(
    size_t id) const {
  return pImpl->textureMap.find(id)->second.allocationRTV;
}

MyDX12::MeshGPUBuffer& RsrcMngrDX12::RegisterStaticMeshGPUBuffer(
...
MyDX12::MeshGPUBuffer& RsrcMngrDX12::RegisterDynamicMeshGPUBuffer(
...
MyDX12::MeshGPUBuffer& RsrcMngrDX12::GetMeshGPUBuffer(size_t id) const {

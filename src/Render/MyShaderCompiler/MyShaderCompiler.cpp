...
struct MyShaderCompiler::Impl {
  class MyShaderCompilerInstance : private details::MyShaderBaseVisitor {
   ...
   private:
    ...
    virtual antlrcpp::Any visitChildren(
      ...
      for (size_t i = 0; i < n; i++) {
        ...
        result =
            std::move(childResult);  // aggregateResult(result, childResult);
      }
    ...
    virtual antlrcpp::Any visitRoot_signature(
      ...
      for (auto paramCtx : ctx->root_parameter()) {
        ...
        base = static_cast<unsigned int>(std::stoull(
            paramCtx->register_index()->shader_register()->getText(), nullptr,
            0));
        if (auto spaceCtx = paramCtx->register_index()->register_space())
          space = static_cast<unsigned int>(
              std::stoull(spaceCtx->getText(), nullptr, 0));
        else
          space = 0;
        ...
        if (auto registerNumCtx = paramCtx->register_num()) {
          ...
          range.NumDescriptors = static_cast<unsigned int>(
              std::stoull(registerNumCtx->getText(), nullptr, 0));
    ...
    virtual antlrcpp::Any visitTags(
      ...
      for (auto tagCtx : ctx->tag()) {
        ...
        curPass->tags.emplace(std::string{StringLiteralContentView(key_s)},
    ...
    virtual antlrcpp::Any visitBlend(
      ...
      if (auto expr = ctx->blend_expr()) {
        if (auto srcCtx = expr->blend_src_factor_color())
          curPass->renderState.blendStates[index].src =
              DecodeBlendStr(srcCtx->getText());
        if (auto destCtx = expr->blend_dst_factor_color())
          curPass->renderState.blendStates[index].dest =
              DecodeBlendStr(destCtx->getText());
        if (auto srcACtx = expr->blend_src_factor_alpha())
          curPass->renderState.blendStates[index].srcAlpha =
              DecodeBlendStr(srcACtx->getText());
        if (auto dstACtx = expr->blend_dst_factor_alpha())
          curPass->renderState.blendStates[index].destAlpha =
              DecodeBlendStr(dstACtx->getText());
      }
    ...
    virtual antlrcpp::Any visitBlend_op(
      ...
      if (auto indexCtx = ctx->index()) {
        ...
        index = 0;
      curPass->renderState.blendStates[index].enable = true;
      curPass->renderState.blendStates[index].op =
          DecodeBlendOpStr(ctx->blend_op_color()->getText());
      if (auto opACtx = ctx->blend_op_alpha())
        curPass->renderState.blendStates[index].opAlpha =
            DecodeBlendOpStr(opACtx->getText());
    ...
    virtual antlrcpp::Any visitQueue(
      ...
      size_t queue;
      if (auto signCtx = ctx->val_queue()->Sign();
          signCtx && signCtx->getText() != "-")
        queue = base + offset;
      else {
        if (base > offset)
          queue = base - offset;

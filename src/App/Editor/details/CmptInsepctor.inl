	...
	void InspectCmpt(Cmpt* cmpt, CmptInspector::InspectContext ctx) {
		if constexpr (HasDefinition<MySRefl::TypeInfo<Cmpt>>::value) {
			...
			if (ImGui::CollapsingHeader(MySRefl::TypeInfo<Cmpt>::name.data())) {
				MySRefl::TypeInfo<Cmpt>::ForEachVarOf(*cmpt, [ctx](auto field, auto& var) {
	...
	void InspectUserType(Field field, UserType* obj, CmptInspector::InspectContext ctx) {
		if constexpr (HasDefinition<MySRefl::TypeInfo<UserType>>::value) {
			if (ImGui::TreeNode(field.name.data())) {
				...
				MySRefl::TypeInfo<UserType>::ForEachVarOf(*obj, [ctx](auto field, auto& var) {
					InspectVar(field, var, ctx);
				});
				ImGui::TreePop();
				ImGui::PopID();
		...
		if constexpr (std::is_same_v<Value, bool>) {
			...
			if constexpr (ValNTraits<Value>::isValN) {
				...
				if (ImGui::TreeNode(field.name.data())) {
					...
					for (size_t i = 0; i < ArrayTraits<Value>::size; i++) {
						...
						InspectVar(MySRefl::Field{ std::string_view{str}, (void*)0 }, ArrayTraits_Get(var, i), ctx);
					}
					ImGui::PopID();
					ImGui::TreePop();
				}
			...
			if (ImGui::TreeNode(field.name.data())) {
				...
				for (auto iter = iter_begin; iter != iter_end; ++iter) {
					...
					if constexpr (std::is_same_v<std::decay_t<decltype(key)>, std::string>)
						InspectVar(MySRefl::Field{ std::string_view{key}, (void*)0 }, mapped, ctx);
					else {
						auto name = std::to_string(key);
						InspectVar(MySRefl::Field{ std::string_view{name}, (void*)0 }, mapped, ctx);
					}
				}
				ImGui::PopID();
				ImGui::TreePop();
			...
			if (ImGui::TreeNode(field.name.data())) {
				...
				for (auto iter = iter_begin; iter != iter_end; ++iter) {
					...
					InspectVar(MySRefl::Field{ std::string_view{name}, (void*)0 }, *iter, ctx);
				}
				ImGui::PopID();
				ImGui::TreePop();
			}
...
namespace My::MyGE {
	...
	void CmptInspector::RegisterCmpt(Func&& func) {
		...
		RegisterCmpt(MyECS::CmptType::Of<Cmpt>, [f = std::forward<Func>(func)](void* cmpt, InspectContext ctx) {

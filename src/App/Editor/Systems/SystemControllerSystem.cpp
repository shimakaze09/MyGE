  ...
  schedule.GetWorld()->AddCommand(
      [w = schedule.GetWorld()]() {
        auto systemController =
            w->entityMngr.WriteSingleton<SystemController>();
        if (!systemController || !systemController->world) return;
        ...
        if (ImGui::Begin("System Controller")) {
          ...
          if (ImGui::BeginPopup("Create System Popup")) {
            ...
            NameID createID;
            const size_t N =
                systemController->world->systemMngr.systemTraits.GetNames()
                    .size();
            for (const auto& [ID, name] :
                 systemController->world->systemMngr.systemTraits.GetNames()) {
              if (!systemController->world->systemMngr.IsAlive(ID) &&
                  filter.PassFilter(name.data())) {
                ImGui::PushID(idx);
                ImGui::PushStyleColor(
                    ImGuiCol_Button,
                    (ImVec4)ImColor::HSV(idx / float(N), 0.6f, 0.6f));
                ImGui::PushStyleColor(
                    ImGuiCol_ButtonHovered,
                    (ImVec4)ImColor::HSV(idx / float(N), 0.7f, 0.7f));
                ImGui::PushStyleColor(
                    ImGuiCol_ButtonActive,
                    (ImVec4)ImColor::HSV(idx / float(N), 0.8f, 0.8f));
                if (ImGui::Button(name.data())) createID = ID;
          ...
          if (ImGui::BeginPopup("Destroy System Popup")) {
            ...
            NameID destroyID;
            size_t N =
                systemController->world->systemMngr.GetAliveSystemIDs().size();
            for (const auto& ID :
                 systemController->world->systemMngr.GetAliveSystemIDs()) {
              auto name =
                  systemController->world->systemMngr.systemTraits.Nameof(ID);
              if (filter.PassFilter(name.data())) {
                ...
                ImGui::PushStyleColor(
                    ImGuiCol_ButtonActive,
                    (ImVec4)ImColor::HSV(idx / float(N), 0.8f, 0.8f));
                if (ImGui::Button(name.data())) destroyID = ID;
                ImGui::PopStyleColor(3);
                ImGui::PopID();
              }
              idx++;
...

          {
            ...
            for (const auto& ID :
              ...
              if (filter.PassFilter(name.data())) {
                bool isActive =
                    systemController->world->systemMngr.IsActive(ID);
                if (ImGui::Checkbox(name.data(), &isActive)) switchID = ID;
              }
            }
            if (switchID.Valid()) {

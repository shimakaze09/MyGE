...
struct ImGuiViewportDataDx12 {
  ...
  ~ImGuiViewportDataDx12() {
    ...
    for (UINT i = 0; i < g_numFramesInFlight; ++i) {
      IM_ASSERT(FrameCtx[i].CommandAllocator == NULL &&
                FrameCtx[i].RenderTarget == NULL);
...
void ImGui_ImplDX12_RenderDrawData(ImDrawData* draw_data,
  ...
  ImGui_ImplDX12_RenderBuffers* fr =
      &render_data
           ->FrameRenderBuffers[render_data->FrameIndex % g_numFramesInFlight];
...
static bool ImGui_ImplDX12_CreateFontsTexture() {
  ...
  // Upload texture to graphics system
  {
    ...
    hr = g_pd3dDevice->CreateFence(0, D3D12_FENCE_FLAG_NONE,
                                   IID_PPV_ARGS(&fence));
    IM_ASSERT(SUCCEEDED(hr));
...
bool ImGui_ImplDX12_CreateDeviceObjects() {
  ...
  // Create the root signature
  {
    ...
    ID3DBlob* blob = NULL;
    if (D3D12SerializeRootSignature(&desc, D3D_ROOT_SIGNATURE_VERSION_1, &blob,
                                    NULL) != S_OK)
      return false;
  ...
  // Create the vertex shader
  {
    ...
    if (FAILED(D3DCompile(vertexShader, strlen(vertexShader), NULL, NULL, NULL,
                          "main", "vs_5_0", 0, 0, &vertexShaderBlob, NULL)))
...
bool ImGui_ImplDX12_Init_Shared(
...
bool ImGui_ImplDX12_Init_Context(ImGuiContext* ctx) {
  ...
  io.BackendFlags |=
      ImGuiBackendFlags_RendererHasViewports;  // We can create multi-viewports on the Renderer side (optional) // FIXME-VIEWPORT: Actually unfinished..

  // Create a dummy ImGuiViewportDataDx12 holder for the main viewport,
  // Since this is created and managed by the application, we will only use the ->Resources[] fields.
  ImGuiViewport* main_viewport =
      ctx->Viewports[0];  // ImGui::GetMainViewport();
...
void ImGui_ImplDX12_Shutdown_Context(ImGuiContext* ctx) {
  ...
  ImGuiViewport* main_viewport =
      ctx->Viewports[0];  // ImGui::GetMainViewport();
  if (ImGuiViewportDataDx12* data =
          (ImGuiViewportDataDx12*)main_viewport->RendererUserData) {
...
static void ImGui_ImplDX12_CreateWindow(ImGuiViewport* viewport) {
  ...
  HRESULT res = S_OK;
  res = g_pd3dDevice->CreateCommandQueue(&queue_desc,
                                         IID_PPV_ARGS(&data->CommandQueue));
  IM_ASSERT(res == S_OK);

  // Create command allocator.
  for (UINT i = 0; i < g_numFramesInFlight; ++i) {
    res = g_pd3dDevice->CreateCommandAllocator(
  ...
  res = g_pd3dDevice->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT,
                                        data->FrameCtx[0].CommandAllocator,
                                        NULL, IID_PPV_ARGS(&data->CommandList));
  IM_ASSERT(res == S_OK);
  data->CommandList->Close();

  // Create fence.
  res = g_pd3dDevice->CreateFence(0, D3D12_FENCE_FLAG_NONE,
                                  IID_PPV_ARGS(&data->Fence));
  IM_ASSERT(res == S_OK);
  ...
  res = dxgi_factory->CreateSwapChainForHwnd(data->CommandQueue, hwnd, &sd1,
  ...
  if (data->SwapChain) {
    ...
    HRESULT hr = g_pd3dDevice->CreateDescriptorHeap(
        &desc, IID_PPV_ARGS(&data->RtvDescHeap));
    IM_ASSERT(hr == S_OK);

    SIZE_T rtv_descriptor_size = g_pd3dDevice->GetDescriptorHandleIncrementSize(
    ...
    for (UINT i = 0; i < g_numFramesInFlight; i++) {
      ...
      g_pd3dDevice->CreateRenderTargetView(
          back_buffer, NULL, data->FrameCtx[i].RenderTargetCpuDescriptors);
...
static void ImGui_WaitForPendingOperations(ImGuiViewportDataDx12* data) {
  ...
  if (data && data->CommandQueue && data->Fence && data->FenceEvent) {
    ...
    hr = data->Fence->SetEventOnCompletion(data->FenceSignaledValue,
                                           data->FenceEvent);
    IM_ASSERT(hr == S_OK);
...
static void ImGui_ImplDX12_DestroyWindow(ImGuiViewport* viewport) {
  ...
  if (ImGuiViewportDataDx12* data =
          (ImGuiViewportDataDx12*)viewport->RendererUserData) {
...
static void ImGui_ImplDX12_SetWindowSize(ImGuiViewport* viewport, ImVec2 size) {
  ...
  if (data->SwapChain) {
    ...
    data->SwapChain->ResizeBuffers(0, (UINT)size.x, (UINT)size.y,
...
static void ImGui_ImplDX12_RenderWindow(ImGuiViewport* viewport, void*) {
  ...
  cmd_list->OMSetRenderTargets(
      1, &data->FrameCtx[back_buffer_idx].RenderTargetCpuDescriptors, FALSE,
      NULL);
  if (!(viewport->Flags & ImGuiViewportFlags_NoRendererClear))
    cmd_list->ClearRenderTargetView(
        data->FrameCtx[back_buffer_idx].RenderTargetCpuDescriptors,
  ...
  data->CommandQueue->ExecuteCommandLists(1,

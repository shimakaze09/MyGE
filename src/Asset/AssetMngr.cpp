...
struct AssetMngr::Impl {
  ...
  struct Asset {
    ...
    const std::type_info& GetTypeInfo() const noexcept { return typeid(*ptr); }
...
#ifdef MY_MYGE_USE_ASSIMP
  static std::shared_ptr<Mesh> AssimpLoadMesh(
...
bool AssetMngr::IsSupported(std::string_view ext) const noexcept {
  return
      ...
      // [images]
      || ext == "png" || ext == "jpg" || ext == "bmp" || ext == "hdr" ||
      ext == "tga"

      // [rendering]
      || ext == "hlsl" || ext == "shader" || ext == "tex2d" ||
      ext == "texcube" || ext == "mat" || ext == "scene";
}
...
bool AssetMngr::Contains(const Object& obj) const {
  return pImpl->assetID2path.find(obj.GetInstanceID()) !=
         pImpl->assetID2path.end();
}
...
std::shared_ptr<Object> AssetMngr::GUIDToAsset(
    const xg::Guid& guid, const std::type_info& type) const {
  const auto& path = GUIDToAssetPath(guid);
  if (path.empty())
    return nullptr;
...
std::shared_ptr<Object> AssetMngr::LoadAsset(
    const std::filesystem::path& path) {
  ImportAsset(path);
  auto target = pImpl->path2assert.find(path);
  if (target != pImpl->path2assert.end())
  ...
  if (ext == ".lua") {
    ...
    auto str = Impl::LoadText(path);
    auto hlsl =
        std::make_shared<HLSLFile>(std::move(str), path.parent_path().string());
    ...
    auto mode =
        static_cast<TextureCube::SourceMode>(texcubeJSON["mode"].GetInt());
    ...
    switch (mode) {
      case TextureCube::SourceMode::SixSidedImages: {
        ...
        for (size_t i = 0; i < 6; i++) {
          ...
          xg::Guid guid{guidstr};
          auto imgTarget = pImpl->guid2path.find(guid);
          images[i] = imgTarget != pImpl->guid2path.end()
                          ? LoadAsset<Image>(imgTarget->second)
                          : nullptr;
        }
      ...
      case TextureCube::SourceMode::EquirectangularMap: {
        ...
        xg::Guid guid{guidstr};
        auto imgTarget = pImpl->guid2path.find(guid);
        equirectangularMap = imgTarget != pImpl->guid2path.end()
                                 ? LoadAsset<Image>(imgTarget->second)
                                 : nullptr;
...
std::shared_ptr<Object> AssetMngr::LoadAsset(const std::filesystem::path& path,
  ...
  } else if (ext == ".obj" || ext == ".ply" && IsSupported("ply")) {
...
bool AssetMngr::CreateAsset(std::shared_ptr<Object> ptr,
                            const std::filesystem::path& path) {
...
std::shared_ptr<Mesh> AssetMngr::Impl::LoadObj(
    const std::filesystem::path& path) {
  ...
  for (size_t s = 0; s < shapes.size(); s++) {
    ...
    ctx.submeshes.emplace_back(ctx.indices.size(),
...
void AssetMngr::Impl::AssimpLoadMesh(MeshContext& ctx, const aiMesh* mesh,
                                     const aiScene* scene) {
...
std::shared_ptr<Mesh> AssetMngr::Impl::AssimpLoadMesh(
    const std::filesystem::path& path) {
  Assimp::Importer importer;
  const aiScene* scene = importer.ReadFile(
      path.string(), aiProcess_JoinIdenticalVertices | aiProcess_FlipUVs |

...
void InputSystem::OnUpdate(Schedule& schedule) {
  ...
  schedule.RegisterJob(
      [](Singleton<Input> input) {
        ...
        input->MouseInDisplay =
            io.MousePos.x >= 0.f && io.MousePos.x <= io.DisplaySize.x &&
        ...
        memcpy(input->MouseDoubleClicked, io.MouseDoubleClicked,
               5 * sizeof(bool));
        memcpy(input->MouseReleased, io.MouseReleased, 5 * sizeof(bool));
        memcpy(input->MouseDownDuration, io.MouseDownDuration,
               5 * sizeof(float));
        memcpy(input->KeysDownDuration, io.KeysDownDuration,
               512 * sizeof(float));
      },
      SystemFuncName);
}

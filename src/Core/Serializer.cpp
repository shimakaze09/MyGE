...
void Serializer::SerializeRecursion(MyDRefl::ObjectView obj,
  ...
  if (ctx.serializer.IsRegistered(obj.GetType().GetID().GetValue()))
    ...
    if (AssetMngr::Instance().Contains(sobj.GetPtr())) {
      ...
      ctx.writer.String(
          AssetMngr::Instance().NameofAsset(sobj.GetPtr()).data());
      ctx.writer.Key(Key::Guid);
      ctx.writer.String(
          AssetMngr::Instance().GetAssetGUID(sobj.GetPtr()).str());
      ctx.writer.EndObject();
    } else
      ctx.writer.String(Key::NotSupport);
...
std::string Serializer::Serialize(const MyECS::World* w,
      ...
      {  // Entities
        for (Entity e : entities) {
          ctx.writer.StartObject();
          {
            ...
            {  // Components
              for (const auto& cmpt :
                   w->entityMngr.Components(e, AccessMode::LATEST))
                Serializer::SerializeRecursion(
                    {Mngr.tregistry.Typeof(cmpt.AccessType()), cmpt.Ptr()},
                    ctx);
            }
            ctx.writer.EndArray();  // components
          }
...
bool Serializer::DeserializeToWorld(MyECS::World* world, string_view json) {
  Document doc;
